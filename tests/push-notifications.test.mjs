...
test("Stage 10 adds safe click routing and app badges without scheduled dispatch", () => {
  ...
  assert.match(workerSource, /showNotification\(title, options\)/);
  assert.match(workerSource, /addEventListener\("notificationclick"/);
  assert.match(workerSource, /ALLOWED_NOTIFICATION_PATHS/);
  assert.match(workerSource, /navigator\.setAppBadge\(1\)/);
  assert.match(applicationSource, /syncApplicationBadge\(alertCount\)/);
  assert.doesNotMatch(workerSource, /dispatch-push-reminders|notification_outbox/);
});
