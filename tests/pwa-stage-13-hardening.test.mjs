...
test("all current web release markers agree", () => {
  assert.match(pwaSource, /const RELEASE = "4\.9\.5"/);
  assert.match(appPage, /app\.js\?v=92/);
  assert.match(appPage, /styles\.css\?v=64/);
  assert.match(appPage, /pwa\.js\?v=46/);
  assert.match(appPage, /Version 4\.9\.5/g);
  assert.match(workerSource, /pwa-shell-v53/);
  assert.match(workerSource, /"\/pwa\.js\?v=46"/);
});
